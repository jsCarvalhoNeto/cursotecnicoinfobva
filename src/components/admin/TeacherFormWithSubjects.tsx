...
export default function TeacherFormWithSubjects({ onSuccess, teacher }: TeacherFormProps) {
  ...
  useEffect(() => {
    const fetchSubjects = async () => {
      ...
      try {
        ...
        if (teacher) {
          const teacherSubjectsResponse = await fetch(`http://localhost:4002/api/teachers/${teacher.id}/subjects`);
  ...
  const getCurrentTeacherSubjects = async (teacherId: string): Promise<string[]> => {
    const currentSubjectsResponse = await fetch(`http://localhost:4002/api/teachers/${teacherId}/subjects`);
  ...
  const associateSubjectToTeacher = async (subjectId: string, teacherId: string) => {
    await fetch(`http://localhost:4002/api/subjects/${subjectId}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
